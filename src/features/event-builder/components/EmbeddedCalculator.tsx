...
export function EmbeddedCalculator({ 
  ...
  const handleContinue = (eventType: string, params: any) => {
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="conference" className="px-4 pb-4">
            <ClientConferenceCalculator 
              ...
              embedded={true}
              onAddToCart={(config: any) => handleContinue('conference', {
                date: config.date,
                days: config.days,
          ...
          <TabsContent value="wedding" className="px-4 pb-4">
            <WeddingCalculator 
              ...
              dictionary={dictionary} 
              embedded={true}
              onAddToCart={(config: any) => handleContinue('wedding', {
                date: config.date,
                days: config.days,
                attendees: config.attendees,
          ...
          <TabsContent value="celebration" className="px-4 pb-4">
            <CelebrationCalculator 
              ...
              dictionary={dictionary} 
              embedded={true}
              onAddToCart={(config: any) => handleContinue('celebration', {
                date: config.date,
                days: config.days,
                attendees: config.attendees,
                venue: config.selectedAddOns.venue.optionId
          ...
          <TabsContent value="retreat" className="px-4 pb-4">
            <RetreatCalculator 
              ...
              dictionary={dictionary} 
              embedded={true}
              onAddToCart={(config: any) => handleContinue('retreat', {
                date: config.date,
                days: config.days,
                attendees: config.attendees,
                venue: config.selectedAddOns.venue.optionId
